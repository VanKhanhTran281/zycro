...
const AddForm: React.FC<ComponentProps> = ({ onClick, onCreateContact }) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const currentDate = dayjs().locale('en').format('MMM DD, YYYY'); // Lấy ngày hiện tại
        const updatedContactData = {
            ...contactData,
            date_added: currentDate,
            last_updated: currentDate
        };
        await dispatch(createContact(updatedContactData));
        onCreateContact();
        resetForm();
        onClick()
    };
        ...
        <>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '81.5%', transform: 'translate(-50%, -50%)', zIndex: 9999, width: "560px", height: '780px', backgroundColor: '#fff' }}>
                <div style={{ height: '64px', display: 'flex', marginLeft: '25px', alignItems: 'center' }}>
                    ...
                    <svg style={{ marginTop: '20px', marginLeft: '20px', cursor: 'pointer' }} onClick={onClick} width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
